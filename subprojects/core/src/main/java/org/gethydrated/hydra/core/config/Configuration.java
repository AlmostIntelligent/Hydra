     ...
     */
        ...
        private List<String> listFromItem(final ConfigurationItem base,
                ...
                if (name.isEmpty() && base.hasChildren()) {
                        for (ConfigurationItem itm : ((ConfigList) base)
                                        .getChilds()) {
                                l.add(itm.getName());
                        }
                        ...
                        for (ConfigurationItem itm : ((ConfigList) base)
                                ...
                                } else if (name.startsWith(itm.getName()
                                                + configSeparator)) {
                                        return listFromItem(
                                                        itm,
                                                        l,
                                                        name.replace(itm.getName()
                                                                        + configSeparator,
                                                                        ""));
                                }
                        }
        ...
        public final void setItem(final ConfigList base, final String name,
                ...
                if (namesplit.length == 1) {
                        ...
                        try {
                                ConfigValue child = (ConfigValue) base
                                                .getChild(name);
                                child.set(value);
                        } catch (ConfigItemNotFoundException e) {
                                /* Value doesn't exists, add new element */
                                base.getChilds().add(
                                                new ConfigValue<Object>(name,
                                                                value));
                        }
                ...
                for (ConfigurationItem i : base.getChilds()) {
                        if (i.hasValue() && i.getName().equals(name)) {
                                return ((ConfigValue<?>) i).value();
                                        ...
                                        && name.startsWith(i.getName()
                                                        + configSeparator)) {
                                return getFromItem(
                                                (ConfigList) i,
                                                name.replace(i.getName()
                                                                + configSeparator,
                                                                ""));
                        }
                }
