...
public class BootStrapper {
	...
	public static void bootstrap(String[] args, File hydraHome) throws Exception {
		List<File> systemJars = new ArrayList<File>();
		
		addFileSet(systemJars, new File(hydraHome, "lib"), ".jar");
		//addFileSet(systemJars, new File(hydraHome, "conf"), ".xml", ".ini", ".properties");
        ...
        Class<?> mainClass = rootLoader.loadClass("org.gethydrated.hydra.launcher.HydraStarter");
        Method mainMethod = mainClass.getMethod("start", String[].class);
        Thread.currentThread().setContextClassLoader(rootLoader);
        mainMethod.invoke(null, new Object[]{args});
	}
	
	private static void addFileSet(List<File> fileSet, File target, final String... endings) {
