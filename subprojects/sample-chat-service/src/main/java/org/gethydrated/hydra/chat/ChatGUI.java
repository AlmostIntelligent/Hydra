...
public final class ChatGUI extends javax.swing.JFrame {
    ...
    private static final long serialVersionUID = -8426368664093216866L;
    ...
    public ChatGUI(final ServiceContext context) {
        ...
        this.users.put(context.getSelf().getUSID(), context.getSelf().getUSID()
                .toString());
    ...
    private void inputKeyPressed(final java.awt.event.KeyEvent evt) {
    ...
    private final HashMap<USID, String> users;
    ...
    private void handleInput() {
        ...
        if (!s.isEmpty()) {
            if (!handleCommands(s)) {
                ...
                for (final USID usid : users.keySet()) {
                    if (!usid.equals(context.getSelf().getUSID())) {
                        final SID sid = context.getService(usid);
    ...
    private boolean handleCommands(final String in) {
        if (in.startsWith("/name ") && in.length() > 6) {
            ...
            for (final USID usid : users.keySet()) {
                if (!usid.equals(context.getSelf().getUSID())) {
                    final SID sid = context.getService(usid);
                    sid.tell(
                            new Renamed(context.getSelf().getUSID(), in
                                    .substring(6, in.length())), context
                                    .getSelf());
                }
            }
            return true;
    ...
    /**
     * Renames a chat client.
     * @param sid client identifier.
     * @param name client name.
     */
    public void renameUser(final SID sid, final String name) {
        if (name != null) {
     ...
     * @param input chat message.
     */
    public void handleInput(final SID from, final String input) {
        chat.append("[" + users.get(from.getUSID()) + "] " + input + "\n");
    }

    @SuppressWarnings("unchecked")
    private void updateUsers() {
        final List<String> names = new LinkedList<>(users.values());
     ...
     */
    public void addClient(final SID sid) {
        if (!users.containsKey(sid.getUSID())) {
            ...
            sid.tell(new NewClient(context.getSelf().getUSID()),
                    context.getSelf());
            sid.tell(
                    new Renamed(context.getSelf().getUSID(), users.get(context
                            .getSelf().getUSID())), context.getSelf());
        }
    }

    /**
     * Removes a chat client.
     * @param sid client id.
     */
    ...
    /**
     * Removes all chat clients.
     * @param uuid client usid.
     */
    public void removeAll(final UUID uuid) {
        final Iterator<USID> it = users.keySet().iterator();
        while (it.hasNext()) {
