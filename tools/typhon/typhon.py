...
class TyphonGUI():
	...
	def __init__(self):
		...
		self._fMenu.add_command(label="Start", command=self.start)
		self._fMenu.add_command(label="Shutdown", command=self.stop)
		self._fMenu.add_command(label="Quit", command=self.close)
		...
		self._logText.tagEdit("warning", foreground="yellow")
		self._logText.tagEdit("note", foreground="green")
